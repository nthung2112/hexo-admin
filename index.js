...
if (passwordProtected) {
  if (!hexo.config.admin.password_hash) {
    ...
    throw new Error(
      '[Hexo Admin]: the provided password_hash looks like an md5 hash -- hexo-admin has switched to use bcrypt; see the Readme for more info.'
    );
  }
...
hexo.extend.filter.register('server_middleware', function(app) {
  ...
  // Main routes
  app.use(hexo.config.root + 'admin/', serveStatic(path.join(__dirname, 'dist')));
