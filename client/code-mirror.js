...
var CodeMirror = createReactClass({
  ...
  componentDidMount: function() {
    ...
    this.cm = CM(ReactDOM.findDOMNode(this), editorSettings);
    ...
    var box = ReactDOM.findDOMNode(this).parentNode.getBoundingClientRect();
    ...
    reader.onload = event => {
      ...
      if (settings.options) {
        if (!!settings.options.askImageFilename) {
          var filePath = !!settings.options.imagePath
            ? settings.options.imagePath
            : '/images';
          filename = prompt(
            `What would you like to name the photo? All files saved as pngs. Name will be relative to ${filePath}.`,
            'image.png'
          );
        }
      }
      ...
      api
        .uploadImage(event.target.result, filename)
        .then(res => this.cm.replaceSelection(`\n![${res.msg}](${res.src})`));
    };
