...
var GrammarSuggestion = createReactClass({
  propTypes: {
  ...
  render: function() {
    ...
    return (
      <div className="grammar_box">
        {suggestion && (
          <pre className="grammar_suggestion">{suggestion.join('\n')}</pre>
        )}
        {reason}
      </div>
    );
...
var suggestionContents = function(suggestions) {
  ...
  if (suggestions.length === 0) {
    ...
    contents = (
      <div className="grammar_box">
        <p className="grammar_reason">
          <i style={golden} className="fa fa-star" />&nbsp;Nice! No possible
...
var CheckGrammar = createReactClass({
  propTypes: {
  ...
  componentDidUpdate: function(prevProps) {
    if (prevProps.raw !== this.props.raw) {
      var suggestions = writeGood.annotate(
        this.props.raw,
        writeGood(this.props.raw)
      );
  ...
  render: function() {
    ...
    return (
      <div className="post-content editor_rendered">
        ...
        <p style={creditStyle}>
          Brought to you by{' '}
          <a href="https://github.com/btford/write-good" target="_blank">
            write-good
          </a>.
        </p>
